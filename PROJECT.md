...
- ~~**Missing AGC stage** (observed 2026-05-18)~~ — **RESOLVED**: WebRTC AGC2 landed as a post-NS stage (own APM instance to preserve ADR-0001 ordering, 2026-05-27) and is **on by default since v0.4.1** (ADR-0012 A6; flips the AEC-Challenge doubletalk verdict to PASS). `--no-agc` opts out; `--agc-max-gain-db` tunes the adaptive-digital cap (sweep showed 20–80 identical on the corpus — cap never engages; WebRTC default 50 kept). Cost: 6 % of chain CPU (per-stage split, 2026-07-04).

- **GB/T 45314 double-talk near-end preservation (China eCall pre-compliance, ADR-0013) — BLOCK as of 2026-07-04.** The gate's doubletalk condition measures near-end at −40 dB below the −12 dB preservation floor (post-NS tap; −24 dB after AGC). Stage-tap evidence localizes the loss to AEC3 nonlinear suppression, upstream of RNNoise — the same gap the AEC-Challenge ST_FE structural dnsmos_sig and the cabin-demo over-suppression point at. B2 no-speech noise stability is solved by the `--ns-dry-blend 0.20` eCall preset candidate. Resolution path is **Phase 3 neural RES**; see `docs/gbt45314-ecall-gate-current-verdict.md`.
